...
def naive_tracking(event, Policy, truth=None):
    ...
    for track_no in range(1, event[0].shape[0]):
        ...
        for layer_no in range(1, event.shape[0]):
            ...
            choice = np.random.choice(distr.shape[0])
            if choice == 0:
                break
            else:
...
def main(argv):
    ...
    random_policy = lambda pairs: tf.zeros(pairs.shape[0], dtype=tf.float32) + 1
    ...
    #    naive_tracks, naive_truth_tracks = naive_tracking(test_data[target], random_policy, test_key[target])
    #    tracks, truth_tracks = tracking(test_data[target], random_policy, Evaluate, test_key[target])
    #    naive_truth_sets.append(naive_truth_tracks)
    #    truth_sets.append(truth_tracks)

    global run_tracking
    global run_naive
    def run_tracking(pair):
        _, truth_tracks = tracking(pair[0], random_policy, Evaluate, pair[1])
        return truth_tracks
    def run_naive(pair):
        _, truth_tracks = naive_tracking(pair[0], random_policy, pair[1])
        return truth_tracks

    with mp.Pool() as p:
        event_tuples = [(test_data[target], test_key[target]) for target in range(test_data.shape[0])]
        truth_sets = p.map(run_tracking, event_tuples, chunksize=10)
        naive_truth_sets = p.map(run_naive, event_tuples, chunksize=10)
    p.join()
    print(truth_sets[0])
